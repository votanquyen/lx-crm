 ...
 * Mapping:
 * - companies → customers
 * - invoices (old) → invoices (new)
 * - monthly_statements → preserved as reference data
 * - plant_items → preserved as reference data
 *
    ...
    try {
      ...
      await prisma.customer.create({
        data: {
          ...
          address: company.address || "Địa chỉ chưa cập nhật",
...
async function migrateInvoices(
  ...
  for (const invoice of invoices) {
    ...
    try {
      ...
      if (!customerId) {
        console.warn(`  ⚠️ No customer for invoice ${invoiceNumber}`);
        skipped++;
        continue;
      }
      ...
      await prisma.invoice.create({
        data: {
          ...
          periodStart,
          periodEnd,
          periodLabel: `Tháng ${String(invoice.month).padStart(2, "0")}/${invoice.year}`,
...
async function main() {
  console.log("🚀 Starting migration from neon_complete.sql to Prisma v4 schema\n");

  // Parse SQL file
  const { companies, invoices } = parseSqlFile();

  // Migrate companies → customers
  await migrateCompaniesToCustomers(companies);

  // Build mapping: old UUID → new CUID
  ...
  console.log(`\n🔗 Mapped ${companyIdMap.size} company IDs`);
